...
module CircleDraw
	def draw(screen, color = [0, 0, 255])
		screen.draw_circle([@x_pivot, @y_pivot], @radius, color)
	end
end

module CircleUI
	def draw_sidebar(screen, fields, color = [0, 255, 0])
		screen.draw_text("Add Circle", 630, 20, color)
		...
		screen.draw_text("Radius", 670, 240, color)
		...
		screen.draw_text("#{fields[:radius]}", 690, 275, color)
	...
	def initialize_temp_variables
		["@fields[:x] = 300", "@fields[:y] = 300", "@fields[:radius] = 150"]
	end
	...
	def handle_mouse_input(x, y)
		...
		decr_rad_button = Rubygame::Rect.new(640, 270, 20, 30)
		incr_rad_button = Rubygame::Rect.new(730, 270, 20, 30)
		...
		if decr_x_button.collide_point?(x, y)
			...
			"@fields[:y] = @fields[:y] + 1"
		elsif decr_rad_button.collide_point?(x, y)
			"@fields[:radius] = @fields[:radius] == 1 ? 1 : @fields[:radius] - 1"
		elsif incr_rad_button.collide_point?(x, y)
			"@fields[:radius] = @fields[:radius] + 1"
		elsif ok_button.collide_point?(x, y)
			"@diagram.add_shape(Circle.new(@fields[:x], @fields[:y], @fields[:radius]))
